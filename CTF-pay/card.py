...
def run(
    ...
    if (
        ...
        try:
            ...
            # passwordless_signup 账号 DB 里 password=""，但登录流程已有
            # passwordless 分支；mail_cfg 足够时仍可通过 OTP 登录换 token。
            if _mail_cfg:
                if _password:
                    _log("      [RT] 支付成功，重新登录拿 Codex token ...")
                else:
                    _log("      [RT] 支付成功，账号无 password (passwordless_signup)，走 OTP 登录路径拿 Codex token ...")
